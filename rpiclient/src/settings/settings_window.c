...
SettingsWindow *new_settings_window(void)
{
    ...
    if (instance)
    {
        ...
        gtk_editable_insert_text(
            GTK_EDITABLE(instance->entry_address), TEXT_EXAMPLE_ENTRY_ADDRESS,
            NEW_TEXT_LENGTH_ENTRY_ADDRESS, &(instance->cur_pos_address)
        );
        gtk_editable_select_region(
            GTK_EDITABLE(instance->entry_address), START_POSITION_ENTRY_ADDRESS,
            GTK_ENTRY(instance->entry_address)->text_length
        );
        ...
        gtk_editable_insert_text(
            GTK_EDITABLE(instance->entry_port), TEXT_EXAMPLE_ENTRY_PORT,
            NEW_TEXT_LENGTH_ENTRY_PORT, &(instance->cur_pos_port)
        );
        gtk_editable_select_region(
            GTK_EDITABLE(instance->entry_port), START_POSITION_ENTRY_PORT,
            GTK_ENTRY(instance->entry_port)->text_length
        );
